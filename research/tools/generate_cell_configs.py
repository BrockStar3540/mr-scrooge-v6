    ...
    for _p, _s, _id in _SL40:
        for st in configs.get(_p, {}).get("sessions", {}).get(_s, {}).get("setups", []):
            if st.get("id") == _id:
                ...
                _trig = 8.5   # Brock 2026-07-19: whole book to engage +8.5 / lock +6 (was 7.5/lock5, Brock 2026-07-15)
        ...
        for _sess, _scfg in _pcfg.get("sessions", {}).items():
            ...
            for _st in _scfg.get("setups", []):
                ...
                _new = {"mode": "ratchet", "sl_pips": _sl, "trigger_pips": _trig, "trail_pips": _trail,
                        "trail_mult": 0.0, "trail_min": _trail, "trail_max": max(_trail * 3.0, 10.0),  # trail_mult=0 -> FIXED trail (Brock: no ATR-scaling; engage +8.5, lock +6)
                        "_class": "RANGE_SIZED"}
