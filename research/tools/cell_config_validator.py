#!/usr/bin/env python3
"""research/tools/cell_config_validator.py — Cell config schema validator.

Usage:
    python cell_config_validator.py config/cells/GBP_USD.json
    python cell_config_validator.py --all          # validates all files in config/cells/
"""
...
def validate_file(path: Path) -> ValidationErrors:
    ...
    pair = _require_str(data, "pair", "top", errs, choices=VALID_PAIRS)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate cell config JSON files.")
    ...
    # Resolve cells directory
    script_dir = Path(__file__).resolve().parent
    repo_root = script_dir.parent.parent  # research/tools -> research -> repo
    cells_dir = Path(args.cells_dir) if args.cells_dir else repo_root / "config" / "cells"

    paths: list[Path] = []
    if args.all:
        if not cells_dir.is_dir():
            print(f"ERROR: cells directory not found: {cells_dir}", file=sys.stderr)
            return 2
        paths = sorted(cells_dir.glob("*.json"))
        if not paths:
            print(f"No *.json files found in {cells_dir}")
            return 0
    elif args.files:
    ...
    if args.all:
        ...
        print(f"\nSummary: {n_ok} OK / {n_fail} FAIL / {len(results)} total")
