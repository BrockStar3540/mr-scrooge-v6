...
Two display defects made the board unreadable as a leaderboard:
  ...
  1. BROKER TRUTH sorted worst-first, so the account's strongest cell sat at
     the BOTTOM of the page and the biggest loser led.
...
def test_one_lucky_cycle_cannot_outrank_a_repeater():
    """The real case: es_trend_long/asia made +$64.99 in a SINGLE cycle;
    control_atr5m_60 made +$67.39 over ELEVEN. Raw dollars put them side by
    side; the shrunk score must not."""
    lucky = {"usd": 64.99, "cycles": 1}
    proven = {"usd": 67.39, "cycles": 11}
...
def test_shrink_is_monotonic_in_cycles():
    ...
    for cyc in (1, 2, 3, 5, 8, 11, 20):
        v = sb._btruth_score({"usd": 100.0, "cycles": cyc})
        if prev is not None:
            assert v > prev
        prev = v
    # never credits more than the dollars actually earned
    assert sb._btruth_score({"usd": 100.0, "cycles": 999}) < 100.0


def test_no_completed_cycle_scores_neutral():
    """judge-when-flat: an unfinished family is neither credited nor condemned."""
    assert sb._btruth_score({"usd": 500.0, "cycles": 0}) == 0.0
    assert sb._btruth_score({"usd": -500.0, "cycles": 0}) == 0.0
    assert sb._btruth_score({"usd": None, "cycles": 4}) == 0.0


def test_losses_shrink_toward_zero_too():
    assert sb._btruth_score({"usd": -100.0, "cycles": 1}) > \
           sb._btruth_score({"usd": -100.0, "cycles": 11})


def test_broker_truth_ranks_best_first(monkeypatch):
    def _fam(setup, usd, cycles):
        return {"instrument": "EUR_JPY", "session": "ny", "setup": setup,
                "n": cycles, "greens": cycles, "usd": usd, "pips": usd,
    ...
    monkeypatch.setitem(sb._FAM_CACHE, "full", {
        ...
        "families": [_fam("big_loser", -220.0, 7),
                     _fam("one_hit_wonder", 64.99, 1),
                     _fam("real_edge", 67.39, 11)],
    ...
    assert [r["setup"] for r in rows] == ["real_edge", "one_hit_wonder", "big_loser"]
